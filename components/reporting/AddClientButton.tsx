...
import { createClient } from '@/app/(dashboard)/reporting/clients/actions'
...
export function AddClientButton() {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const res = await createClient(trimmed, long === '' ? null : long)
      if (res.ok) {
        setOpen(false)
        reset()
        router.push(`/reporting/clients/${res.id}`)
      } else {
        setError(res.error)
      }
    })
  ...
  return (
    <>
      ...
      <Modal open={open} title="New client" onClose={close}>
        <form onSubmit={submit}>
          <div style={{ marginBottom: 12 }}>
            <label style={fieldLabel}>Short name *</label>
            <input
              type="text"
              value={name}
